...
public class СoncurrentExecutor : IExecutor
{
    public async Task DemoAsync()
    {
        ...
        Task producer = Task.Run(() => ProduceData(queue, dictionary, bag, blockingCollection));
        Task consumer = Task.Run(() => ConsumeData(queue, dictionary, bag, blockingCollection));
    ...
    static void ProduceData(ConcurrentQueue<int> queue, ConcurrentDictionary<int, string> dictionary, ConcurrentBag<int> bag, BlockingCollection<int> blocking)
    {
        for (int i = 0; i < 10; i++)
        {
            ...
            // Add to blockingCol
            blocking.TryAdd(i);
            Console.WriteLine($"Добавлено в blocking: {i}");
    ...
    /// <param name="dictionary"></param>
    /// <param name="bag"></param>
    static void ConsumeData(ConcurrentQueue<int> queue, ConcurrentDictionary<int, string> dictionary, ConcurrentBag<int> bag, BlockingCollection<int> blocking)
    {
        for (int i = 0; i < 10; i++)
        {
            ...
            {
                blocking.TryTake(out var blockItem);
                Console.WriteLine($"Извлечено из blocking: {blockItem}");
            }
    ...
    public long CalculateSum(int n, int m)
    {
        ...
        // CalculateWithConcurrentDictionary(n,  m);
         CalculateWithBlockingCollection(n, m);
    }
    ...
    static long CalculateWithBlockingCollection(int n, int m)
    {
        var block = new BlockingCollection<long>();
